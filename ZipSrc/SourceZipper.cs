...
internal class SourceZipper
{
    ...
    public static async ValueTask ZipAsync(string targetDir, string? targetZipPath = default, CompressionLevel compressionLevel = CompressionLevel.SmallestSize, Action<LogLevel, string>? outputLog = null)
    {
        // Determine target zip path
        targetZipPath = DetermineTargetZipPath(targetDir, targetZipPath);
        outputLog?.Invoke(LogLevel.Normal, $"Creating zip archive at: {Path.GetRelativePath(Environment.CurrentDirectory, targetZipPath)}");

        // Prepare ignore lists
        var rootIgnoreList = await GetDefaultIgnoreList(targetDir, outputLog);
    ...
    private static async ValueTask<IEnumerable<IgnoreList>> GetDefaultIgnoreList(string targetDir, Action<LogLevel, string>? outputLog)
    {
        ...
        outputLog?.Invoke(LogLevel.Detailed, "Use default .gitignore generated by 'dotnet new gitignore'");
    ...
    private static void ZipFolderRecursively(ZipContext context, string folderPath, IEnumerable<IgnoreList> ignoreLists)
    {
        ...
        if (targetFolderState == IgnoreState.Denied)
        {
            context.OutputLog?.Invoke(LogLevel.Detailed, $"Skip denied folder: {Path.GetRelativePath(context.TargetDir, folderPath)}");
            return;
        }
        if (IsWindows && targetFolderState != IgnoreState.Accepted && new DirectoryInfo(folderPath).Attributes.HasFlag(FileAttributes.Hidden))
        {
            context.OutputLog?.Invoke(LogLevel.Detailed, $"Skip hidden folder: {Path.GetRelativePath(context.TargetDir, folderPath)}");
            return;
        }
        ...
        if (gitIgnoreExists)
        {
            context.OutputLog?.Invoke(LogLevel.Detailed, $"Load .gitignore in folder: {Path.GetRelativePath(context.TargetDir, folderPath)}");
            ignoreLists = ignoreLists.Prepend(IgnoreList.Create(folderPath, gitIgnorePath));
        }
        ...
        if (File.Exists(gitInfoExcludePath))
        {
            context.OutputLog?.Invoke(LogLevel.Detailed, $"Load .git/info/exclude in folder: {Path.GetRelativePath(context.TargetDir, folderPath)}");
            ignoreLists = ignoreLists.Prepend(IgnoreList.Create(folderPath, gitInfoExcludePath));
        }
        ...
        foreach (var filePath in files)
        {
            ...
            if (ignireState == IgnoreState.Denied)
            {
                context.OutputLog?.Invoke(LogLevel.Detailed, $"Skip denied file: {Path.GetRelativePath(context.TargetDir, filePath)}");
                continue;
            }
            if (IsWindows && ignireState != IgnoreState.Accepted && new FileInfo(filePath).Attributes.HasFlag(FileAttributes.Hidden))
            {
                context.OutputLog?.Invoke(LogLevel.Detailed, $"Skip hidden file: {Path.GetRelativePath(context.TargetDir, filePath)}");
                continue;
            }

            var entryName = Path.GetRelativePath(context.TargetDir, filePath);
            context.OutputLog?.Invoke(LogLevel.Normal, $"Adding file: {entryName}");
